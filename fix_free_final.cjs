      ...
      <main className="flex-1 p-8 overflow-y-auto custom-scrollbar space-y-6">
        ...
        <div className="flex justify-between items-end mb-8">
          ...
          <div className="flex gap-4">
            <div className="bg-white border border-zinc-200 px-6 py-3 rounded-2xl shadow-sm text-center">
              ...
              <p className="text-xl font-black italic tracking-tighter text-zinc-900">184 <span className="text-[10px] not-italic text-zinc-400">GÜN</span></p>
            </div>
          </div>
        </div>
        ...
        <div className="grid grid-cols-12 gap-6">
          ...
          <div className="col-span-8 bg-white border border-zinc-200 rounded-[2.5rem] p-8 shadow-sm">
            <div className="flex justify-between items-center mb-6">
              <h3 className="text-[11px] font-black uppercase tracking-widest text-zinc-400 flex items-center gap-2">
                <Clock className="w-4 h-4"/> Günlük Ders Akışı
              </h3>
              <button className="text-[9px] font-black text-blue-600 uppercase hover:underline transition-all">Tüm Program</button>
            </div>
            ...
            <div className="space-y-3">
              <div className="p-4 bg-zinc-50 rounded-2xl border border-zinc-100 flex justify-between items-center group cursor-pointer hover:border-blue-200 transition-all">
                <div className="flex items-center gap-4">
                  ...
                  <div>
                    <h4 className="text-sm font-bold text-zinc-800 uppercase tracking-tight">Anatomi: Üst Ekstremite Kasları</h4>
                    <p className="text-[10px] font-bold text-zinc-400 uppercase mt-0.5">Amfi 1 • Prof. Dr. Ahmet Y.</p>
                  </div>
                </div>
                <span className="text-xs font-black tabular-nums text-zinc-400 group-hover:text-blue-600 transition-colors">08:30 - 10:20</span>
              </div>
            </div>
          </div>

          {/* Sağ Kolon: Sınav Odak Kartı */}
          <div className="col-span-4 bg-zinc-900 rounded-[2.5rem] p-8 text-white relative overflow-hidden shadow-xl group cursor-pointer">
            ...
            <h3 className="text-[10px] font-black uppercase tracking-widest text-zinc-500 mb-10 italic">Sıradaki Engel</h3>
            <div className="relative z-10">
              <h4 className="text-2xl font-black italic tracking-tighter leading-none mb-2 text-white">1. KOMİTE:<br/>HÜCRE BİLİMİ</h4>
              <p className="text-4xl font-black text-blue-500 italic mt-6">21 <span className="text-xs not-italic text-zinc-500 uppercase">Gün Kaldı</span></p>
            </div>
          </div>

          {/* Alt Satır: Araçlar ve Hızlı Erişim */}
          <div className="col-span-12 grid grid-cols-4 gap-6 mt-2">
            <div className="bg-white border border-zinc-200 rounded-[2rem] p-6 shadow-sm hover:shadow-md transition-all cursor-pointer group">
              <div className="w-10 h-10 bg-zinc-100 rounded-xl flex items-center justify-center mb-4 group-hover:bg-blue-50">
                <Bookmark className="w-5 h-5 text-zinc-400 group-hover:text-blue-500 transition-colors" />
              </div>
              <h4 className="text-[11px] font-black uppercase text-zinc-800">Slayt Listesi</h4>
              <p className="text-[9px] font-bold text-zinc-400 uppercase mt-1 tracking-widest">4 Dosya Bekliyor</p>
            </div>

            <div className="bg-white border border-zinc-200 rounded-[2rem] p-6 shadow-sm hover:shadow-md transition-all cursor-pointer group">
              <div className="w-10 h-10 bg-zinc-100 rounded-xl flex items-center justify-center mb-4 group-hover:bg-amber-50">
                <Zap className="w-5 h-5 text-zinc-400 group-hover:text-amber-500 transition-colors" />
              </div>
              <h4 className="text-[11px] font-black uppercase text-zinc-800">Spot Bilgiler</h4>
              <p className="text-[9px] font-bold text-zinc-400 uppercase mt-1 tracking-widest">Günün 10 Spotu</p>
            </div>

            <div className="bg-white border border-zinc-200 rounded-[2rem] p-6 shadow-sm hover:shadow-md transition-all cursor-pointer group">
              <div className="w-10 h-10 bg-zinc-100 rounded-xl flex items-center justify-center mb-4 group-hover:bg-emerald-50">
                <Beaker className="w-5 h-5 text-zinc-400 group-hover:text-emerald-500 transition-colors" />
              </div>
              <h4 className="text-[11px] font-black uppercase text-zinc-800">Lab Değerleri</h4>
              <p className="text-[9px] font-bold text-zinc-400 uppercase mt-1 tracking-widest">Referans Rehberi</p>
            </div>

            <div className="bg-blue-600 rounded-[2rem] p-6 shadow-xl shadow-blue-900/20 flex flex-col justify-between group cursor-pointer overflow-hidden relative">
              ...
              <div className="flex items-center gap-2 relative z-10 text-white font-black text-[11px] uppercase italic">
                Sistemi Yükselt <ChevronRight className="w-3 h-3" />
              </div>
            </div>
          </div>
...
console.log('✅ Kod Cerrahi Olarak Onarıldı: Mükerrer Bookmark ve Dışarıda Kalan JSX Temizlendi!');
